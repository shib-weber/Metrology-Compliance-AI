...
export default function Navbar() {
  ...
  return (
    <header className="relative z-50 w-full shrink-0 bg-slate-950/95 backdrop-blur-md border-b border-slate-800/80">
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-indigo-400 font-mono font-normal tracking-wider">
              {isDashboardRoute ? 'PORTAL' : ''}
            </span>
          </div>
        </Link>

        {/* Navigation Rail - Hidden on dashboard routes to avoid UI clutter */}
        {!isDashboardRoute && (
        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated ? (
            <div className="flex items-center gap-3">
              ...
              {!isDashboardRoute && (
                user?.role === 'inspector' ? (
                  <Link 
                    to="/inspector" 
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-700/60 bg-slate-800/40 text-slate-200 text-sm font-medium hover:bg-slate-800 transition"
                  >
              ...
              <button 
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700/60 bg-slate-800/40 text-rose-300 hover:text-rose-200 hover:bg-slate-800 text-sm font-medium transition"
                title="Log out"
              >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <Link 
                to="/login" 
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition shadow-md shadow-indigo-600/20"
              >
          ...
          {isAuthenticated ? (
            ...
            <div className="flex flex-col gap-2 pt-3 border-t border-slate-800">
              ...
              <Link 
                to="/signup" 
                onClick={() => setOpen(false)} 
                className="flex items-center justify-center px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium mt-2"
              >
                Register Account
              </Link>
            </div>
